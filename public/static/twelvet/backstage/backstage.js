...
! function () {
	...
	window.twelvet = {
		...
		init: function () {
			// 获取本地储存官网登录信息
			let twelvet_user = window.localStorage.getItem('twelvet');
			// 拥有信息连接初始化登录状态
			if (twelvet_user) {
				twelvet.T.status = true;
				twelvet.T.user = JSON.parse(twelvet_user);
			}
			// 初始化layui引入
			twelvet.layui.init();
			$('a[data-toggle="checkupdate"]', '.justify-content-end').on('click', twelvet.update)
			//检测屏幕宽度
		...
		menuDate: function (o) {
			...
			// 无法获取text将获取title属性
			let title = $.trim(o.text()) ? $.trim(o.text()) : $.trim(o.attr('title'));
