...
class Http{
    ...
    private static function useCurl($url, $method, $charset='UTF-8', $dataArr = []){
		...
		curl_setopt($cloudServer, CURLOPT_URL, $url);   //处理地址
        ...
        // 检查是否有错误发生
        if(curl_errno($cloudServer)){ return ['state'=>false, 'msg'=>'Error（'. curl_error($cloudServer) .'）']; }
        ...
        //检查状态码是否正常
        if($httpCode != 200){ return ['state'=>false, 'msg'=>'UseCurl：返回状态'. $httpCode]; }
        //判断获取的内容是否为空
        if (strlen($data) == 0){ return ['state'=>false, 'msg'=>'UseCurl：获取内容为空']; }
        //返回正常数据
        return ['state'=>true, 'msg'=>$data];
    }
